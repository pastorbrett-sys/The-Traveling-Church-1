...
const denominations = [
  ...
  },
  {
    id: "other",
    label: "Other",
    description: "Specify your tradition below",
    icon: () => (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6">
...
export function OnboardingTradition() {
    ...
    <div className="min-h-screen w-full flex flex-col" style={{ background: "hsl(30, 20%, 97%)", fontFamily: "'Poppins', system-ui, sans-serif" }}>
      ...
      <div className="px-6 pt-10 pb-2">
        <div className="flex items-center gap-2 mb-6">
          {[1, 2, 3].map((step) => (
            <div key={step} className="flex items-center gap-2">
              ...
              {step < 3 && (
                <div className="w-8 h-0.5 rounded" style={{ background: step === 1 ? "#B78D00" : "hsl(30, 15%, 90%)" }} />
              )}
            </div>
          ))}
        </div>
        ...
        <p className="text-sm" style={{ color: "hsl(20, 10%, 40%)" }}>
          This helps us address your pastor the right way.
        </p>
      </div>
        ...
        {denominations.map((d) => {
          ...
          return (
            <div key={d.id} className="space-y-2">
              ...
              {/* Other text input */}
              {isSelected && d.id === "other" && (
                <div className="px-1">
                  <Input
                    placeholder="e.g. Anglican, Lutheran, Non-denominational..."
                    value={otherText}
                    onChange={(e) => setOtherText(e.target.value)}
                    className="h-12 rounded-xl text-sm"
                    style={{ borderColor: "#B78D00" }}
                  />
                </div>
              )}
            </div>
