...
- **Regional Pricing**: Two-tier pricing model ($7.99 Premium for developed markets, $1.99 Emerging for developing markets). Card country detection ensures tourists pay premium pricing. Config: `shared/regionalPricing.ts`, endpoints: `/api/pricing/tier`, `/api/stripe/regional-checkout`. Requires: `STRIPE_PRICE_PRO_PREMIUM` and `STRIPE_PRICE_PRO_EMERGING` environment variables.
