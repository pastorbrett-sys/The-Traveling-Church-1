...
- **Regional Pricing**: Two-tier pricing model ($7.99 Premium for developed markets, $1.99 Emerging for developing markets). Device locale detection (not IP) ensures tourists pay premium pricing based on where their phone was set up. Config: `shared/regionalPricing.ts`, endpoints: `/api/pricing/tier`, `/api/stripe/regional-checkout`. Requires: `STRIPE_PRICE_PRO_PREMIUM` and `STRIPE_PRICE_PRO_EMERGING` environment variables.
