...
- **Sermon Transcription** (Pro-only): Record audio sermons or voice notes directly in-app, transcribe via OpenAI Whisper, review/edit the text, and save as notes. Pro users get 4 transcriptions per month (tracked in `feature_usage` table). Uses browser MediaRecorder API which works natively in iOS/Android WebView. iOS microphone permission configured in Info.plist.
