          ...
          <td align="center">
            <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width: 600px; background-color: #ffffff;" bgcolor="#ffffff">
              ...
              <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: #ffffff; border-radius: 12px; border: 1px solid #e5e5e5; margin: 0 0 30px 0;">
                <tr>
                  <td style="padding: 20px; text-align: left;">
                    <p style="font-size: 15px; line-height: 1.5; color: #333; margin: 0 0 12px 0;"><span style="display: inline-block; width: 24px; height: 24px; background-color: #C99A2E; color: white; border-radius: 50%; text-align: center; line-height: 24px; font-size: 14px; font-weight: 600; margin-right: 12px; vertical-align: middle;">1</span> Open the App or Site</p>
                    <p style="font-size: 15px; line-height: 1.5; color: #333; margin: 0 0 12px 0;"><span style="display: inline-block; width: 24px; height: 24px; background-color: #C99A2E; color: white; border-radius: 50%; text-align: center; line-height: 24px; font-size: 14px; font-weight: 600; margin-right: 12px; vertical-align: middle;">2</span> Go to the Menu</p>
                    <p style="font-size: 15px; line-height: 1.5; color: #333; margin: 0 0 12px 0;"><span style="display: inline-block; width: 24px; height: 24px; background-color: #C99A2E; color: white; border-radius: 50%; text-align: center; line-height: 24px; font-size: 14px; font-weight: 600; margin-right: 12px; vertical-align: middle;">3</span> Tap "Ambassadors"</p>
                    <p style="font-size: 15px; line-height: 1.5; color: #333; margin: 0;"><span style="display: inline-block; width: 24px; height: 24px; background-color: #C99A2E; color: white; border-radius: 50%; text-align: center; line-height: 24px; font-size: 14px; font-weight: 600; margin-right: 12px; vertical-align: middle;">4</span> Copy your referral link and share it!</p>
                  </td>
                </tr>
              </table>
...
export function getDonationReceiptEmailHtml(
  donorName: string,
      ...
      <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff;">
        ...
        <div style="padding: 10px 30px 50px; background-color: #FAF9F6;">
          ...
          <div style="background-color: #ffffff; border: 1px solid #e5e5e5; border-radius: 12px; padding: 24px; margin-bottom: 28px;">
            ...
            <table style="width: 100%; font-size: 15px; color: #333;" cellpadding="0" cellspacing="0">
              ...
              <tr>
                ...
                <td style="padding: 8px 0; text-align: right; font-weight: 600; color: #C99A2E; font-size: 18px;">${amountFormatted}</td>
              </tr>
              <tr>
                <td style="padding: 8px 0; color: #888;">Type</td>
                <td style="padding: 8px 0; text-align: right; font-weight: 600;">${frequencyLabel}</td>
              </tr>
              ${note ? `
              <tr>
                <td style="padding: 8px 0; color: #888;">Note</td>
                <td style="padding: 8px 0; text-align: right; font-style: italic;">${note}</td>
              </tr>
              ` : ''}
            </table>
          </div>
          ...
          <p style="font-size: 14px; line-height: 1.6; color: #555; text-align: center; margin: 0 0 24px 0;">
            ${isMonthly ? 'Your monthly gift will continue to make an impact. You can manage your recurring donation anytime.' : 'Your one-time gift makes a real difference. Thank you for standing with us.'}
          </p>
...
export async function sendDonationReceiptEmail(
  ...
  console.log(`[Email] Sending donation receipt to ${donorEmail} ($${(amountCents / 100).toFixed(2)}, ${frequency})...`);
  ...
  try {
    ...
    const subject = isMonthly
      ? `Monthly Donation Receipt — ${amountFormatted}/mo | The Traveling Church`
      : `Donation Receipt — ${amountFormatted} | The Traveling Church`;

    const result = await client.emails.send({
      from: fromEmail,
      to: donorEmail,
      subject,
      html: getDonationReceiptEmailHtml('Donor', amountFormatted, frequency, donationDate, note),
    });

    console.log('[Email] Donation receipt sent:', JSON.stringify(result));
    return result;
  } catch (error) {
    console.error('[Email] Error sending donation receipt:', error);
    throw error;
  }
}
